 ...
 */
Schema.process = function(schemaMap, schemaPath, docDir, schemaDir, meta, readme, docs) {
  smap = schemaMap;
  ...
  return Promise.mapSeries(keys, schemaKey => {
    ...
    return Schema.getExamples(schemaMap[schemaKey].filePath, schema)
      ...
      .then(allSchema => {
        ...
        const outputTasks = [ asciidocWriter(schemaMap[schemaKey].filePath, object.mSchema, schemaPath, docDir, object.dep, docs) ];
        if (schemaDir !== '') {
    ...
    if (readme) {
      ...
      return readmeWriter(asciidocs, schemaMap, docDir, schemaPath);
    } else {
