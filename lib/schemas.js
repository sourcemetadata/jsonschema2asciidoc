...
const formats = [undefined, 'date-time', 'date', 'time', 'email', 'idn-email', 'hostname', 'idn-hostname', 'ipv4', 'ipv6', 'uri', 'uri-reference', 'iri', 'iri-reference', 'uri-template', 'json-pointer', 'relative-json-pointer', 'regex']

const mergeFormats = Object.fromEntries(formats.map(format => {
  return [format, Object.fromEntries(formats.map(format => {
    return [format, null]
  }));]
}));

mergeFormats['email']['idn-email'] = 'idn-email'
mergeFormats['hostname']['idn-hostname'] = 'idn-hostname'
mergeFormats['uri']['uri-reference'] = 'uri-reference'
mergeFormats['iri']['iri-reference'] = 'iri-reference'
mergeFormats['uri-reference']['uri-template'] = 'uri-template'
mergeFormats['iri-reference']['uri-template'] = 'uri-template'
mergeFormats['json-pointer']['relative-json-pointer'] = 'relative-json-pointer'
mergeFormats.each {
