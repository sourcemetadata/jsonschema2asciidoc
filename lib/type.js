...
class MultipleType extends Type {
  constructor(baseTypes) {
    ...
    } else if (flatBaseTypes.every(baseType => {
      return baseType instanceof NullableType;
    })) {
      return new NullableType(new MultipleType(flatBaseTypes.map(baseType => {
        return baseType.baseType;
      })));
    }
    let foundNumber = false;
    let indexOfInteger = -1;
    let foundNumberArray = false;
    let indexOfIntegerArray = -1;
    ...
    flatBaseTypes.forEach(newBaseType => {
      ...
      } else if (newBaseType instanceof ArrayType && newBaseType.baseType === SimpleType.number) {
        if (!foundNumberArray) {
          foundNumberArray = true;
          if (indexOfIntegerArray > -1) {
            newFlatBaseTypes[indexOfIntegerArray] = newBaseType;
          }
        } else {
          return;
        }
      } else if (newBaseType instanceof ArrayType && newBaseType.baseType === SimpleType.integer) {
        if (indexOfIntegerArray = -1) {
          if (foundNumberArray) {
            return;
          }
          indexOfIntegerArray = newFlatBaseTypes.length;
        } else {
          return;
        }
      }
